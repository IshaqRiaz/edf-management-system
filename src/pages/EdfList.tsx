...
export const EdfList: React.FC<EdfListProps> = ({
  ...
  const getContextualExportFilename = (prefix = 'EDF_Requisition_Report', ext = 'csv') => {
    const parts = [prefix];
    if (selectedCategory && selectedCategory !== 'All') {
      const catClean = selectedCategory.replace(/[^a-zA-Z0-9]/g, '_').replace(/_+/g, '_');
      parts.push(catClean);
    }
    if (selectedStatus && selectedStatus !== 'All') {
      parts.push(selectedStatus);
    }
    if (searchTerm && searchTerm.trim()) {
      const searchClean = searchTerm.trim().slice(0, 16).replace(/[^a-zA-Z0-9]/g, '_');
      parts.push(`filter_${searchClean}`);
    }
  ...
  const handleExportExcel = () => {
    if (filteredEdfs.length === 0) {
      onToast?.('warning', 'No Records to Export', 'There are no requisitions matching your current filters.');
      return;
    }
    const filename = getContextualExportFilename('EDF_Requisition_Report', 'xlsx');
    exportEdfsToExcel(filteredEdfs, filename);
    onToast?.(
      'success',
      'Excel Report Exported',
      `Exported ${filteredEdfs.length} filtered records to "${filename}".`
    );
  };
  ...
  const handleExportCsv = () => {
    ...
    const filename = getContextualExportFilename('EDF_Filtered_Report', 'csv');
    exportEdfsToCsv(filteredEdfs, filename, {
    ...
    onToast?.(
      ...
      `Exported ${filteredEdfs.length} filtered EDF record${filteredEdfs.length > 1 ? 's' : ''} to "${filename}".`
    );
  };
  ...
  const handleExportSelectedCsv = () => {
    ...
    const filename = `Selected_EDF_Records_${selectedEdfs.length}_items_${new Date().toISOString().split('T')[0]}.csv`;
    exportEdfsToCsv(selectedEdfs, filename);
    onToast?.(
      'success',
      'Selected Records Exported',
      `Exported ${selectedEdfs.length} selected EDF records to "${filename}".`
    );
  };

  const handleExportSelectedExcel = () => {
    if (selectedEdfs.length === 0) return;
    const filename = `Selected_EDF_Records_${selectedEdfs.length}_items_${new Date().toISOString().split('T')[0]}.xlsx`;
    exportEdfsToExcel(selectedEdfs, filename);
    onToast?.(
      'success',
      'Selected Excel Exported',
      `Exported ${selectedEdfs.length} selected EDF records to "${filename}".`
    );
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          <button
            ...
            disabled={filteredEdfs.length === 0}
            title={
              filteredEdfs.length > 0
                ? `Export ${filteredEdfs.length} current filtered EDF records to CSV for reporting`
                : 'No filtered records to export'
            }
            aria-label="Export current filtered EDF list to CSV"
            className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-xl bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/60 shadow-2xs hover:border-slate-300 dark:hover:border-slate-600 transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
          >
            <Download className="w-3.5 h-3.5 text-rose-500" />
            <span>Export Filtered CSV</span>
            <span className="ml-0.5 px-1.5 py-0.2 rounded-full text-[10px] font-bold bg-rose-50 dark:bg-rose-950/60 text-rose-600 dark:text-rose-400 border border-rose-200/60 dark:border-rose-900/50">
              {filteredEdfs.length}
            </span>
          </button>
          <button
            ...
            title={
              filteredEdfs.length > 0
                ? `Download current filtered list as Excel (.xlsx)`
                : 'No filtered records to export'
            }
            className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-xl bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/60 shadow-2xs hover:border-slate-300 dark:hover:border-slate-600 transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
          >
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-100 dark:border-slate-800 text-xs">
          ...
          <div className="flex flex-wrap items-center gap-2.5 text-xs">
            {searchTerm.trim() ? (
              ...
              <span className="text-slate-500 dark:text-slate-400">
                Showing <strong className="text-slate-900 dark:text-white font-bold">{filteredEdfs.length}</strong> of {edfs.length} requisitions
              </span>
            )}
            ...
            <button
              onClick={handleExportCsv}
              disabled={filteredEdfs.length === 0}
              title="Download the currently filtered requisitions as a CSV report"
              className="inline-flex items-center gap-1 px-2.5 py-1 text-[11px] font-semibold rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed shadow-2xs"
            >
              <Download className="w-3 h-3 text-[#FF5A5F]" />
              <span>Export Filtered CSV</span>
            </button>
          </div>
        </div>
        ...
        <div className="p-3.5 rounded-2xl bg-gradient-to-r from-rose-50 via-orange-50 to-rose-50 dark:from-slate-900 dark:via-slate-850 dark:to-slate-900 border border-rose-200 dark:border-rose-900/60 flex flex-wrap items-center justify-between gap-3 shadow-xs animate-in fade-in slide-in-from-top-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-xl bg-slate-800 hover:bg-slate-700 text-white dark:bg-slate-700 dark:hover:bg-slate-600 shadow-xs transition-all hover:scale-105 active:scale-95 cursor-pointer"
              title={`Export ${selectedCount} selected records as CSV`}
            >
              <Download className="w-3.5 h-3.5 text-[#FF5A5F]" />
              <span>Export Selected CSV ({selectedCount})</span>
            </button>
